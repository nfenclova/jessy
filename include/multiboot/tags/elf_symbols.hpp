...
public:
  ...
  public:
    ...
    section_flags const & flags() const noexcept
    {
      return m_flags;
    }
    void const * const & address() const noexcept
    {
      return m_virtualAddress;
    }
    uint64_t const & size() const noexcept
    {
      return m_size;
    }
  ...
  {
    return reinterpret_cast<entry const *>(&m_data);
  }
...
#define CASE(Enumerator)                                                                                                       \
  case elf_symbols::section_type::Enumerator:                                                                                  \
    return #Enumerator;
    switch (type)
    {
      CASE(unused)
